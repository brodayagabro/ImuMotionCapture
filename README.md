...
управляет потоком ESP32 и поддерживает калибровку A (руки вниз) → T →
«руки вперёд» → P (ладони вместе, внутренний угол локтя 45°) → A.
Также доступны полная IMU-калибровка и отдельный семафорный мастер XZ:
A-поза → А → У → Ж → Е → Д → A-поза, один круг с автоматической записью
и проверкой возврата в A-позу. Слегка согнутое правое предплечье в Д не используется
как угловой эталон. Подробности — в [README приложения](host/viz/pyqt_mocap/README.md).
