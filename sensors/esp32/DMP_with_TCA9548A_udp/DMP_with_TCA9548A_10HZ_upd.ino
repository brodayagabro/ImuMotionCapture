...
void addSensor(uint8_t channel) {
  ...
  // Keep the physical TCA9548A channel as the stable over-the-air sensor ID.
  s.id = channel;
  s.channel = channel;
  s.dmpReady = false;
  s.hasQuat = false;
...
void calibrateAllSensors(bool calibrateAccelerometers) {
  ...
  for (uint8_t i = 0U; i < sensorCount; i++) {
    ...
    if (calibrateAccelerometers) {
      // I2Cdevlib CalibrateAccel targets X=0, Y=0, Z=+1 g.
      mpu.CalibrateAccel(CALIBRATION_LOOPS);
    }
    mpu.CalibrateGyro(CALIBRATION_LOOPS);
...
void handleCommand(char* cmd, const IPAddress* remoteIp, uint16_t remotePort) {
  ...
  } else if (strcasecmp(cmd, "CALIB_GYRO") == 0) {
