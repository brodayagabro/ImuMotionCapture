    ...
    def _try_capture_neutral(self, now_s: float) -> bool:
        ...
        try:
            for segment_name, sample in samples.items():
                captured[segment_name] = mapped_sensor_quaternion(
                    sample.quaternion, self.axis_maps[segment_name]
                )
        except ValueError:
            return False
    ...
    def _update_segment_deltas(self) -> bool:
        ...
        for segment_name, sensor_id in self.sensor_mapping.items():
            ...
            if sample is None or self.applied_generation.get(segment_name) == sample.generation:
                continue
            current = mapped_sensor_quaternion(sample.quaternion, self.axis_maps[segment_name])
...
def compute_body_pose(orientations: Mapping[str, Quaternion]) -> BodyPose:
    ...
    rest_directions = {
        "spine": spine_vector,
        "shoulder.L": left_upper_vector,
        "forearm.L": forearm_vector,
        "shoulder.R": right_upper_vector,
        "forearm.R": forearm_vector,
    }
    axis_origins = {
        name: (start + end) * 0.5 for name, (start, end) in tracked.items()
    }
    axis_frames = {
        name: rotations[name] @ _segment_rest_basis(rest_directions[name])
        for name in SEGMENT_NAMES
    }
    joints = (
        pelvis,
