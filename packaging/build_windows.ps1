...
$SpecPath = Join-Path $PSScriptRoot "pyqt_mocap.spec"
...
try {
    ...
    $OldSingleFilePath = Join-Path $ProjectRoot "dist\pyqt_mocap.exe"
    ...
    $ExecutablePath = Join-Path $ProjectRoot "dist\pyqt_mocap\pyqt_mocap.exe"
    if (-not (Test-Path -LiteralPath $ExecutablePath)) {
        throw "Build completed without the expected executable: $ExecutablePath"
    }

    Write-Host "Windows build created: $ExecutablePath"
}
finally {
    Pop-Location
}
